...
- (void) select {
    ...
    sqlite3 *db = NULL;
    result = [self open:&db mode:SQLITE_OPEN_READONLY];
    if( result==SQLITE_OK ){
        sqlite3_stmt *stmt = NULL;

        NSString *cmd = @"select key,value from %@;";
        NSString *sql = [NSString stringWithFormat:cmd, mTableName];
        result = sqlite3_prepare_v2(db, [sql UTF8String], -1, &stmt, NULL);
        if( result==SQLITE_OK ){
             while( SQLITE_DONE!=sqlite3_step(stmt)){
                char *keyAsCString   = (char *)sqlite3_column_text(stmt, 0); // 0-index
                char *valueAsCString = (char *)sqlite3_column_text(stmt, 1);

                NSString *key   = [[NSString alloc] initWithUTF8String:keyAsCString];
                NSString *value = [[NSString alloc] initWithUTF8String:valueAsCString];
